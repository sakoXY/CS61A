...
def insert_into_all(item, nested_list):
    ...
    for list_item in nested_list:
        ans += [list_item[:]]
    for list_item in ans:
        list_item.insert(0, item)
    return ans
...
def subseqs(s):
    ...
    if not s:
        ...
        insert_seq = subseqs(s[1:])
        return insert_seq+insert_into_all(s[0], insert_seq)
...
def insert(link, value, index):
    ...
    if index == 0:
        link_new = Link(link.first, link.rest)
        link.first = value
        link.rest = link_new
        return
    else:
...
def long_paths(t, n):
    ...
    ans = []
    for c in t.branches:
        temp = long_paths(c, n-1)
        for item in temp:
            item.insert(0, t.label)
        ans += temp
    return ans
