...
class ProfessorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        oppo_atk = opponent_card.attack
        oppo_def = opponent_card.defense
        ...
        for i in range(orig_opponent_deck_length):
            if copy_oppo_deck[i].attack == opponent_card.attack or copy_oppo_deck[i].defense == opponent_card.defense:
                index_to_del.append(i)
        opponent.deck.cards = [copy_oppo_deck[i] for i in range(
            orig_opponent_deck_length) if i not in index_to_del]
