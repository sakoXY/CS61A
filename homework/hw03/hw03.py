...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_coins_helper(change, smallest_coin):
        if change < 0 or smallest_coin == None:
            return 0
        if change == 0:
            return 1
        with_smallest = count_coins_helper(
            change - smallest_coin, smallest_coin)
        without_smallest = count_coins_helper(
            change, get_next_coin(smallest_coin))
...
def move_stack(n, start, end):
    ...
    def move_disk(start_peg, end_peg):
        print("Move the top disk from rod {} to rod {}".format(start_peg, end_peg))
    if n == 1:
        move_disk(start, end)
    else:
        spare_disk = 6-start-end
        move_stack(n-1, start, spare_disk)
        move_disk(start, end)
        move_stack(n-1, spare_disk, end)
