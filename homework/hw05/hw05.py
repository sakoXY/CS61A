...
class VendingMachine:
    ...
    def add_funds(self, coin):
        ...
        if self.stock:
            ...
            self.change = 0
            return f"Inventory empty. Restocking required. Here is your ${coin}."
    ...
    def vend(self):
        if self.stock == 0:
            ...
            return f"You must add ${self.price-self.change} more funds."
        elif self.change == self.price:
            self.stock -= 1
            self.change = 0
            ...
            self.stock -= 1
            change_have = self.change
            self.change = 0
            return f"Here is your {self.name} and ${change_have-self.price} change."
...
def path_yielder(t, value):
    ...
    if t.label == value:
        yield [value]
    for c in t.branches:
        for temp_value in path_yielder(c, value):
            yield [t.label] + temp_value
            "*** YOUR CODE HERE ***"
...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        ...
        return is_bst(left_child) and is_bst(right_child) and bst_max(left_child) <= t.label and bst_min(right_child) > t.label
    else:
        return False
