...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selected_item = [item for item in paragraphs if select(item)]
    return selected_item[k] if k < len(selected_item) else ''
    # END PROBLEM 1
...
def about(topic):
    ...
    def f(unchecked):
        unchecked = split(remove_punctuation(unchecked.lower()))
        for item in topic:
            for unchecked_item in unchecked:
                if item == unchecked_item:
                    return True
        return False
    return f
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words == []:
        ...
        count = sum(1 if x == y else 0 for (x, y)
                    in zip(typed_words, reference_words))
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        diff_list = [diff_function(typed_word, word, limit)
...
def sphinx_switches(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def count_num(starts, goals, count):
        if count > limit:
            return limit + 1
        elif not starts and not goals:
            return count
        elif not starts or not goals:
            return count_num(starts[1:], goals[1:], count+1)
        elif starts[0] == goals[0]:
            return count_num(starts[1:], goals[1:], count)
        else:
            return count_num(starts[1:], goals[1:], count+1)

    return count_num(start, goal, 0)
    # END PROBLEM 6
...
def time_per_word(times_per_player, words):
    ...
    for times_player in times_per_player:
        time_interval.append([times_player[x]-times_player[x-1]
                             for x in range(1, len(times_player))])
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    fastest_answer = [[] for _ in player_indices]
    for word_num in word_indices:
        min_time = float('inf')
        player = 0
        for player_index in player_indices:
            if time(game, player_index, word_num) < min_time:
                min_time = time(game, player_index, word_num)
                player = player_index
        fastest_answer[player].append(word_at(game, word_num))
    return fastest_answer
    # END PROBLEM 10
