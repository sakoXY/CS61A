...
def piggy_points(score):
    ...
    def min_number(x):
        ...
        while x > 0:
            num_current = x % 10
            mini_number = min(num_current, mini_number)
            x = x // 10
        return mini_number
    return 3 + min_number(score ** 2)
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    os = str(opponent_score) if opponent_score >= 10 else '0' + \
        str(opponent_score)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score1 = score1 + \
                take_turn(strategy1(score1, score0), score0, dice, goal)
            if not more_boar(score1, score0):
                who = next_player(who)
    # END PROBLEM 5
    # (note that the indentation for the problem 6 prompt (***YOUR CODE HERE***) might be misleading)
    # BEGIN PROBLEM 6
    # 一个小坑，每轮都要打印，故而放到while里面
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    # 有个注意点就是local variable,注意局部变量running_high和running_high_latest

    def say(score0, score1):
        score_current = score0 if who == 0 else score1
        running_high_lateset = running_high
        if score_current >= last_score:
            running = score_current - last_score
            if running > running_high:
                running_high_lateset = running
                print('Player', who, 'has reached a new maximum point gain.',
                      running_high_lateset, 'point(s)!')
        return announce_highest(who, score_current, running_high_lateset)
    return say
...
def piggypoints_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    return 0 if piggy_score >= cutoff else num_rolls
    # END PROBLEM 10
